...
def transform(cars: list, TODAY_STR: str):
    ...
    for car in cars:
        ...
        t_car["_sourceURL"] = (
            f"https://www.dubizzle.com.eg/ad/{car.get('slug_l1')}-ID{car['externalID']}.html"
        )
...
def change_dtypes(cars: list) -> pd.DataFrame:
    for car in cars:
        if car.get("createdAt") is not None:
            car["createdAt"] = datetime.fromtimestamp(
                car["createdAt"], tz=timezone.utc
            ).isoformat()
        if car.get("updatedAt") is not None:
            car["updatedAt"] = datetime.fromtimestamp(
                car["updatedAt"], tz=timezone.utc
            ).isoformat()
        car["Price"] = (
            int(car["Price"].replace(",", "").split(".")[0])
...
def lambda_handler(event, context):
    ...
    end_date = datetime.strptime(event.get("end_date", str(today)), "%Y-%m-%d").date()
