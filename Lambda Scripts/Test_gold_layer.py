...
REQUIRED_COLS = [
    "id", "externalID", "title", "Price", "Year", "Kilometers",
    "Brand", "Model", "Transmission Type", "Body Type", "Fuel Type",
    "createdAt", "updatedAt", "_ingestionDate",
    "first_seen_date", "scd_valid_from", "scd_valid_to", "status",
]
...
def test(name: str, category: str, severity: str = "error"):
    ...
    def decorator(fn: Callable[[pd.DataFrame], list[str]]):
        _TESTS.append({"name": name, "category": category, "severity": severity, "fn": fn})
        return fn
    return decorator
...
def check_active_scd_valid_to(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} active row(s) have a non-null scd_valid_to (they look closed but status=active). "
...
def check_closed_scd_valid_to(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} closed row(s) have null scd_valid_to. "
            f"Sample IDs: {bad['id'].head(5).tolist()}"]
...
def check_date_order(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} rows where scd_valid_from > scd_valid_to (time went backwards). "
            f"Sample IDs: {bad['id'].head(5).tolist()}"]
...
def check_first_seen_vs_valid_from(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} rows where first_seen_date > scd_valid_from. "
            f"Sample IDs: {bad['id'].head(5).tolist()}"]
...
def check_no_duplicate_active(df: pd.DataFrame) -> list[str]:
    ...
    return [f"{len(dup_ids)} listing ID(s) have more than one active row — SCD integrity broken. "
...
def check_valid_from_not_future(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} rows have scd_valid_from in the future (> {today}). "
...
def check_price_range(df: pd.DataFrame) -> list[str]:
    ...
    if not too_low.empty:
        issues.append(f"{len(too_low)} rows with Price < {PRICE_MIN_EGP:,} EGP (possibly junk). "
                      f"Min seen: {priced['Price'].min():,.0f}")
    if not too_high.empty:
        issues.append(f"{len(too_high)} rows with Price > {PRICE_MAX_EGP:,} EGP. "
...
def check_year_range(df: pd.DataFrame) -> list[str]:
    ...
    if not too_old.empty:
        issues.append(f"{len(too_old)} rows with Year < {YEAR_MIN}. Min: {yeard['Year'].min():.0f}")
    if not too_new.empty:
        issues.append(f"{len(too_new)} rows with Year > {YEAR_MAX}. Max: {yeard['Year'].max():.0f}")
    return issues
...
def check_km_range(df: pd.DataFrame) -> list[str]:
    ...
    if not extreme.empty:
        issues.append(f"{len(extreme)} rows with Kilometers > {KM_MAX:,}. "
...
def check_hp_range(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} rows with implausible Power (hp) values (0 < hp <= {HP_MAX} expected). "
...
def check_cc_range(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} rows with implausible Engine Capacity values (0 < CC <= {CC_MAX} expected). "
...
def check_date_formats(df: pd.DataFrame) -> list[str]:
    ...
    for col in ['first_seen_date', 'scd_valid_from', 'scd_valid_to', '_ingestionDate']:
        ...
        if bad_count:
            issues.append(f"Column '{col}': {bad_count} values that don't parse as YYYY-MM-DD. "
...
def check_no_ancient_dates(df: pd.DataFrame) -> list[str]:
    ...
    for col in ['first_seen_date', 'scd_valid_from']:
        ...
        if not bad.empty:
            issues.append(f"Column '{col}': {len(bad)} dates before {MIN_LAUNCH_DATE}. "
...
def check_feature_dtypes(df: pd.DataFrame) -> list[str]:
    ...
    for col in feature_cols:
        ...
        if dtype not in ('boolean', 'bool'):
            ...
            issues.append(f"Column '{col}' has dtype '{dtype}' instead of boolean. "
                          f"Sample values: {sample_vals}")
    return issues


@test("feature_* values are only True/False/NaN (no string 'True')", "Value Sanity", severity="warning")
def check_feature_no_string_true(df: pd.DataFrame) -> list[str]:
    feature_cols = [c for c in df.columns if c.startswith('feature_')]
    issues = []
    for col in feature_cols:
        if df[col].dtype == object:
            string_trues = df[col].isin(['True', 'False', 'true', 'false', 'TRUE', 'FALSE']).sum()
            if string_trues > 0:
                issues.append(f"'{col}': {string_trues} values stored as string 'True'/'False' "
...
def check_active_count(df: pd.DataFrame) -> list[str]:
    ...
    if n == 0:
        return ["Zero active listings — something is very wrong (mass-delisting or bad data?)"]
    return []
...
def check_active_not_closed(df: pd.DataFrame) -> list[str]:
    ...
    if active_with_to.empty:
        return []
    return [f"{len(active_with_to)} active listings already have a closing date — "
...
def check_deleted_have_to(df: pd.DataFrame) -> list[str]:
    deleted_without_to = df[(df['status'] == STATUS_DELETED) & df['scd_valid_to'].isna()]
    if deleted_without_to.empty:
        return []
    return [f"{len(deleted_without_to)} deleted listings have no closing date — "
...
def check_delisted_after_seen(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} deleted listings have scd_valid_to < first_seen_date. "
            f"Sample IDs: {bad['id'].head(5).tolist()}"]


@test("Listings active for unreasonably long time", "Business Logic", severity="warning")
def check_suspiciously_old_actives(df: pd.DataFrame) -> list[str]:
    ...
    if old.empty:
        return []
    return [f"{len(old)} active listings have been live for more than 365 days. "
            f"Oldest: {active['_days'].max()} days. Could be genuine or a data gap issue."]


@test("No impossible Kilometers for the listed Year", "Business Logic", severity="warning")
def check_km_vs_year(df: pd.DataFrame) -> list[str]:
    ...
    if bad.empty:
        return []
    return [f"{len(bad)} listings have Kilometers implausibly high for their model year "
...
def check_id_valid_from_unique(df: pd.DataFrame) -> list[str]:
    ...
    return [f"{n} rows share the same (id, scd_valid_from). "
            f"Sample: {sample}"]
...
def run_tests(df: pd.DataFrame, verbose: bool = False) -> dict:
    results = []
    category_order = ["Schema", "SCD Integrity", "Completeness", "Value Sanity",
...
def print_report(results: list[dict], df: pd.DataFrame):
    ...
    print("\n" + "═" * 70)
    print(f"  Results: {passed}/{total} passed  |  {errors} error(s)  |  {warnings} warning(s)")
    print("═" * 70 + "\n")
